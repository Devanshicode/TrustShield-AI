...
from features import extract, explain, FEATURE_NAMES

app = Flask(__name__); CORS(app)
def load(p): return joblib.load(p) if os.path.exists(p) else None
URL_BUNDLE, SMS_MODEL = load('url_model.joblib'), load('sms_model.joblib')
...
def scan_url(url):
    f = extract(url)
    p = float(URL_BUNDLE['model'].predict_proba([[f[k] for k in FEATURE_NAMES]])[0][1])
    score = int(round(p * 100))
    if p >= URL_BUNDLE['threshold']: score = max(score, 60)
    reasons = explain(url, f)
    g = safe_browsing(url)
    if g: score = 100; reasons.insert(0, 'Google Safe Browsing lists this link as dangerous')
    layer2 = 'Known bad (Safe Browsing)' if g else 'Clean' if g is False else 'Not configured'
    return {'score': min(score, 100), 'reasons': reasons, 'layer2': layer2, 'ml_probability': round(p, 3)}
