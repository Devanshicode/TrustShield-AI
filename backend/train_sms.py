...
if len(df) > 200 and min_class_count >= 2:
    Xtr, Xte, ytr, yte = train_test_split(
        df['text'], df['label'], test_size=0.2, random_state=42,
        stratify=df['label'] if min_class_count >= 5 else None
    )
else:
    Xtr, Xte, ytr, yte = df['text'], df['text'], df['label'], df['label']
    print("WARNING: dataset too small or a class has <2 examples - test metrics below are not meaningful")
